...
Sana verilen kullanıcı profili ve eşleşen program bilgilerini, döküman bağlamını da göz önüne alarak değerlendirmeli ve bir 'Recommendation' analizi çıkarmalısın.
...
Çıktı JSON Şeması:
{{
  "summary": "Kullanıcıya bu teşviğin ne sunduğunu ve şirketin ihtiyacına nasıl çözüm olacağını açıklayan 3-4 cümlelik profesyonel özet.",
  "why_matched": [
    "Kullanıcı profilinin bu programla neden tam örtüştüğünü gösteren somut analiz maddeleri (en az 2 madde)."
  ],
  "eligibility_notes": [
    "Kullanıcının bu program kapsamında dikkat etmesi gereken kritik şartlar, kısıtlamalar ve riskler (en az 2 madde)."
  ],
  "application_steps": [
    "Adım adım başvuru süreci yönlendirmeleri."
  ],
  "required_documents": [
    "Başvuru esnasında talep edilecek resmi belgeler."
  ]
}}"""
...
def build_overall_answer_prompt(profile: UserProfile, recommendations: List[Recommendation]) -> str:
    ...
    return f"""Kullanıcı için en uygun teşvikleri belirledik ve listeledik. Şimdi kullanıcıya hitaben profesyonel ve rehberlik edici bir giriş yazısı hazırlamalısın.
...
Görevin:
Kullanıcıya hitaben Türkçe bir giriş yazısı oluştur. Bu desteklerin firmanın hedeflerine nasıl katkı sunacağını kısaca belirt. Yazının sonunda her bir teşviğin detaylı analiz raporunun aşağıda sunulduğunu ifade et."""


# services/rag_service/prompt_builder.py içindeki ilgili fonksiyon:
...
def build_answer_prompt(
    ...
    if focused_program_name:
        focus_instruction = f"\n⚠️ ÖNEMLİ: Kullanıcı şu anda özellikle '{focused_program_name}' programı hakkında soru sormaktadır. Cevabını bu program çerçevesinde şekillendir ve dışına çıkmamaya özen göster."

    return f"""Sen resmi devlet destekleri ve teşvik dökümanları üzerine çalışan güvenilir bir asistanısın.
Sana iletilen döküman parçalarını (Bağlam) kullanarak kullanıcının sorusunu en doğru ve anlaşılır biçimde cevaplamalısın.
{focus_instruction}
...
1. Eğer sorunun cevabı "Resmi Bağlam" içerisinde bulunuyorsa, sadece buradaki resmi verilere sadik kalarak açıklayıcı ve profesyonel bir yanıt hazırla.
2. EĞER sorunun cevabı dökümanlarda (Bağlam) YOKSA veya yetersiz kalıyorsa:
   - Kendi genel bilgi birikimini kullanarak kullanıcıya en yararlı ve profesyonel yanıtı sun.
...
3. Yanıt dilini her zaman profesyonel, yapıcı ve Türkçe tut. Markdown elementlerini kullanarak okunabilirliği maksimize et."""